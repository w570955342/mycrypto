...
//封装加密拷贝文件函数
func CtrEncryptFile(src, des string) {
	f1, err1 := os.Open(src)
	...
	for {
		...
		buf = cipher.AesCtrEncrypt(buf[:n], key)
		_, err4 := f2.Write(buf)
		if err4 != nil {
...
//封装解密拷贝文件函数
func CtrDecryptFile(src, des string) {
	...
	for {
		...
		buf = cipher.AesCtrDecrypt(buf[:n], key)
		_, err4 := f2.Write(buf)
		if err4 != nil {
			fmt.Println("f2.Write err:", err4)
			return
...
//对路径的读取进一步封装
func CtrEncryptFileAll() {
	os.MkdirAll("./加密/", 0777)
	fmt.Println("请输入要AES加密的文件的路径：")
lable:
	//获取路径，兼容路径中含有空格
	var src string
	var arr []rune
	...
	defer f.Close()
	fileInfo, err := f.Stat()
	fmt.Println("文件正在进行AES加密，请稍安勿躁……………")
	CtrEncryptFile(src, "./加密/AE"+fileInfo.Name())
	fmt.Println("文件AES加密成功！")
}


func CtrDecryptFileAll() {

	os.MkdirAll("./解密/", 0777)
	fmt.Println("请输入要AES解密的文件的路径：")
	lable:
	//获取路径，兼容路径中含有空格
	var src string
	var arr []rune
	...
	defer f.Close()
	fileInfo, err := f.Stat()
	fmt.Println("文件正在进行AES解密，请稍安勿躁……………")
	CtrDecryptFile(src, "./解密/"+fileInfo.Name()[2:])
	fmt.Println("文件AES解密成功！")
}
